...
class MediaController extends Controller
{
    ...
    public function guardarData(Request $request)
    {
        ...
        if ($validatedMedia['status'] != false) {
            if ($request->media_id > 0) {
                ...
                return response()->json(['mensaje' => 'Archivos guardados con éxito', 'media_id' => $media2->_id, 'preference_id' => $preference->id]);
            }
        } else {
    ...
    public function store(Request $request)
    {
        ...
        if ($request->media_id != '') {
            ...
            if ($tramo[0]->duracion > $request->duration) {
                ...
                if ($request->amount === '0') {
                    return  response()->json(['message' => 'Media registrada correctamente']);
                } else {
                    return  response()->json(['message' => 'Media registrada correctamente', 'preference_id' => $preference]);
                }
            } else {
    ...
    public function show($id)
    {
        ...
        if ($data) {
            ...
            if (is_array(json_decode($data['files_name'], true))) {
                ...
                return response()->json(['data' => $data, 'extension' => $ext], 200);
            } else {
            ...
            return response()->json(['message' => 'No se encontró ningún dato para el ID proporcionado'], 404);
        }
    }
    ...
    public function searchProgramation(Request $request)
    {
        ...
        return response()->json(['data' => $data, 'tramo' => $arr3, 'data_tramo' => $arr1], 200);
    }
    ...
    public function disabledMedia($id)
    {
        ...
        if (!empty($data)) {
            if ($data->isActive == 1) {
                $data->isActive = 0;
                $data->save();
                return response()->json(['message' => 'Desactivado con exito'], 200);
            } else {
                return response()->json(['message' => 'Media desactivada, no puede inactivar una media mas de una vez'], 400);
            }
        } else {
    ...
    public function approved($id)
    {
        ...
        if (!empty($data)) {
            if ($data->approved == 2) {
                $data->approved = 1;
                $data->save();
                return response()->json(['message' => 'Aprobado con exito'], 200);
            } else {
                return response()->json(['message' => 'Media ya aporbada, no puede aprobar una media mas de una vez'], 400);
            }
        } else {
    ...
    public function notApproved($id)
    {
        ...
        if (!empty($media)) {
            if ($media->approved == 2) {
                ...
                return response()->json(['message' => 'Media desaprobada con exito, se envio un email, notificando al cliente'], 200);
            } else {
                return response()->json(['message' => 'Media desaporbada, no puede desaprobar una media mas de una vez'], 400);
            }
        } else {
            return response()->json(['message' => 'Media no encontrada'], 404);
        }
        ...
        if (!empty($media)) {
            if ($media->reproducido !== 1) {
                ...
                if ($rep == 1) {
                    ...
                    return response()->json(['message' => 'Ocurrio un error en la actualizacion'], 400);
                }
            } else {
                return response()->json(['message' => 'La media ya se encuentra actualizada'], 400);
            }
        } else {
            return response()->json(['message' => 'Media no encontrada'], 404);
        }
